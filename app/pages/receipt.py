"""
Halaman publik Receipt (/receipt) - PERSIS seperti /pickup-intake, dengan
perbedaan: (1) nama field/tombol jadi "Receipt", (2) Nama Instansi input
manual biasa (bukan combobox Pickup Center), (3) tiket yang terbentuk masuk
ke data PUSAT (bukan Pickup Center), (4) ada field Catatan tambahan setelah
Keluhan Pelanggan, (5) ada kotak centang Notifikasi Receipt via WhatsApp.
Endpoint API-nya (POST /api/v1/public/receipt) ada di main.py.
"""
import os
...
def receipt_intake_page():
    ...
    <head>
        ...
        <title>Receipt Perangkat - Omron Healthcare</title>
        ...
        <style>
            * { box-sizing: border-box; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
            ...
            .notif-box { background: #e8f0fe; border: 1px solid #bcd4f7; border-radius: 8px; padding: 12px 14px; margin-bottom: 14px; }
            .notif-box label { display: flex; align-items: center; gap: 8px; font-weight: normal; margin-bottom: 0; cursor: pointer; }
            .notif-box input[type=checkbox] { width: auto; }
        </style>
    </head>
    <body>
        <div class="wrap">
            <div class="card">
                ...
                <div class="subtitle">Form Receipt Perangkat</div>

                <div id="successBox" class="success-box">
                    ✅ Receipt berhasil dicatat!
                    <div class="ticket-no" id="successTicketNo"></div>
                    Simpan nomor tiket ini untuk pelanggan.
                    <div style="margin-top:12px;"><button onclick="resetForm()">+ Input Receipt Baru</button></div>
                </div>
                ...
                <form id="intakeForm">
                    ...
                    <div class="form-group">
                        ...
                        <input id="pInstansi" placeholder="Nama instansi (opsional)">
                    </div>
                    ...
                    <div class="form-group"><label>Catatan</label><input id="pNotes" placeholder="Catatan tambahan (opsional)"></div>

                    <div class="notif-box">
                        <label><input type="checkbox" id="pNotifWhatsapp"> Notifikasi Receipt via WhatsApp</label>
                    </div>

                    __TURNSTILE_WIDGET__

                    <button type="submit" id="receiptSubmitBtn">Simpan Receipt</button>
                </form>
            </div>
        </div>
        ...
        <script>
            ...
            document.getElementById('intakeForm').addEventListener('submit', async function(e) {
                ...
                const payload = {
                    ...
                    notif_whatsapp: document.getElementById('pNotifWhatsapp').checked,
                    turnstileToken: (window.turnstile && document.querySelector('.cf-turnstile')) ? turnstile.getResponse() : null,
                };

                try {
                    const res = await fetch('/api/v1/public/receipt', {
                        method: 'POST',
