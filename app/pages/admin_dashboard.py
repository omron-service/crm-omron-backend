"""
Halaman admin panel (/admin) - dipisah dari main.py supaya main.py tidak
jadi satu file raksasa. File ini MURNI berisi template HTML/CSS/JS statis
(dashboard, Data Service, Payment, Inventory Part, Laporan, Setting) - tidak
ada logic bisnis di sini, semua logic ada di endpoint-endpoint API terpisah
yang dipanggil oleh JavaScript di dalam halaman ini.
"""
...
def admin_dashboard_page():
    ...
    <body>
        ...
        <script>
            ...
            // CATATAN: sebelumnya Pickup Center punya daftar Repair Status SENDIRI
            // yang berbeda dari Pusat/Cabang (isinya soal tahapan pengiriman spt
            // "Diteruskan ke Omron", dll). Ini SEKARANG dihapus & disamakan dengan
            // Pusat/Cabang, karena tahapan pengiriman itu SUDAH dipindah ke field
            // terpisah "Status Pengiriman" yang jalan OTOMATIS (bukan dipilih staff
            // manual lagi) - supaya Repair Status tetap konsisten & tidak
            // membingungkan teknisi di lokasi mana pun.
...
const PROVINCE_CITY_DATA = {"Aceh": ["Banda Aceh", "Langsa", "Lhokseumawe", "Sabang", "Meulaboh", "Aceh Besar", "Aceh Utara", "Aceh Tengah"], "Sumatera Utara": ["Medan", "Binjai", "Pematangsiantar", "Tebing Tinggi", "Sibolga", "Tanjungbalai", "Padang Sidempuan", "Deli Serdang", "Karo"], "Sumatera Barat": ["Padang", "Bukittinggi", "Padang Panjang", "Payakumbuh", "Sawahlunto", "Solok", "Pariaman", "Agam"], "Riau": ["Pekanbaru", "Dumai", "Kampar", "Bengkalis", "Indragiri Hulu", "Indragiri Hilir", "Rokan Hulu", "Rokan Hilir"], "Kepulauan Riau": ["Batam", "Tanjungpinang", "Bintan", "Karimun", "Natuna", "Lingga"], "Jambi": ["Jambi", "Sungai Penuh", "Batanghari", "Bungo", "Kerinci", "Merangin", "Muaro Jambi"], "Sumatera Selatan": ["Palembang", "Lubuklinggau", "Pagar Alam", "Prabumulih", "Ogan Komering Ilir", "Ogan Komering Ulu", "Musi Banyuasin", "Musi Rawas"], "Bangka Belitung": ["Pangkal Pinang", "Bangka", "Bangka Barat", "Bangka Tengah", "Bangka Selatan", "Belitung", "Belitung Timur"], "Bengkulu": ["Bengkulu", "Rejang Lebong", "Bengkulu Utara", "Bengkulu Selatan", "Kepahiang", "Kaur"], "Lampung": ["Bandar Lampung", "Metro", "Lampung Selatan", "Lampung Tengah", "Lampung Utara", "Lampung Timur", "Tulang Bawang", "Pesawaran"], "DKI Jakarta": ["Jakarta Pusat", "Jakarta Utara", "Jakarta Barat", "Jakarta Selatan", "Jakarta Timur", "Kepulauan Seribu"], "Jawa Barat": ["Bandung", "Bekasi", "Bogor", "Depok", "Cimahi", "Sukabumi", "Tasikmalaya", "Cirebon", "Banjar", "Karawang", "Purwakarta", "Subang", "Garut", "Ciamis"], "Banten": ["Serang", "Tangerang", "Tangerang Selatan", "Cilegon", "Pandeglang", "Lebak"], "Jawa Tengah": ["Semarang", "Surakarta", "Salatiga", "Magelang", "Pekalongan", "Tegal", "Purwokerto", "Kudus", "Klaten", "Sukoharjo", "Boyolali", "Sragen", "Cilacap"], "DI Yogyakarta": ["Yogyakarta", "Sleman", "Bantul", "Kulon Progo", "Gunungkidul"], "Jawa Timur": ["Surabaya", "Malang", "Kediri", "Madiun", "Blitar", "Mojokerto", "Pasuruan", "Probolinggo", "Batu", "Sidoarjo", "Gresik", "Jember", "Banyuwangi", "Tuban"], "Bali": ["Denpasar", "Badung", "Gianyar", "Tabanan", "Buleleng", "Karangasem", "Klungkung", "Bangli", "Jembrana"], "Nusa Tenggara Barat": ["Mataram", "Bima", "Lombok Barat", "Lombok Tengah", "Lombok Timur", "Lombok Utara", "Sumbawa", "Dompu"], "Nusa Tenggara Timur": ["Kupang", "Ende", "Maumere", "Manggarai", "Manggarai Barat", "Sumba Timur", "Sumba Barat", "Timor Tengah Selatan"], "Kalimantan Barat": ["Pontianak", "Singkawang", "Sambas", "Kubu Raya", "Ketapang", "Sanggau", "Sintang"], "Kalimantan Tengah": ["Palangka Raya", "Kotawaringin Barat", "Kotawaringin Timur", "Kapuas", "Barito Utara", "Barito Selatan"], "Kalimantan Selatan": ["Banjarmasin", "Banjarbaru", "Banjar", "Barito Kuala", "Tanah Laut", "Hulu Sungai Utara", "Hulu Sungai Selatan"], "Kalimantan Timur": ["Samarinda", "Balikpapan", "Bontang", "Kutai Kartanegara", "Kutai Timur", "Kutai Barat", "Berau", "Paser"], "Kalimantan Utara": ["Tarakan", "Bulungan", "Malinau", "Nunukan", "Tana Tidung"], "Sulawesi Utara": ["Manado", "Bitung", "Tomohon", "Kotamobagu", "Minahasa", "Minahasa Utara", "Minahasa Selatan"], "Gorontalo": ["Gorontalo", "Boalemo", "Bone Bolango", "Gorontalo Utara", "Pohuwato"], "Sulawesi Tengah": ["Palu", "Poso", "Banggai", "Donggala", "Toli-Toli", "Parigi Moutong", "Morowali"], "Sulawesi Barat": ["Mamuju", "Majene", "Polewali Mandar", "Mamasa", "Pasangkayu"], "Sulawesi Selatan": ["Makassar", "Parepare", "Palopo", "Gowa", "Maros", "Bone", "Bulukumba", "Pinrang", "Wajo", "Sidenreng Rappang"], "Sulawesi Tenggara": ["Kendari", "Baubau", "Kolaka", "Konawe", "Muna", "Bombana", "Wakatobi"], "Maluku": ["Ambon", "Tual", "Maluku Tengah", "Maluku Tenggara", "Buru", "Seram Bagian Barat"], "Maluku Utara": ["Ternate", "Tidore Kepulauan", "Halmahera Barat", "Halmahera Utara", "Halmahera Tengah", "Halmahera Selatan"], "Papua": ["Jayapura", "Keerom", "Sarmi", "Biak Numfor", "Jayawijaya", "Nabire", "Mimika", "Merauke"], "Papua Barat": ["Manokwari", "Sorong", "Fakfak", "Kaimana", "Teluk Bintuni", "Teluk Wondama", "Raja Ampat"], "Papua Barat Daya": ["Sorong", "Sorong Selatan", "Tambrauw", "Maybrat", "Raja Ampat"], "Papua Tengah": ["Nabire", "Paniai", "Mimika", "Puncak", "Puncak Jaya", "Dogiyai", "Deiyai"], "Papua Pegunungan": ["Jayawijaya", "Pegunungan Bintang", "Yahukimo", "Tolikara", "Yalimo", "Lanny Jaya", "Nduga"], "Papua Selatan": ["Merauke", "Boven Digoel", "Mappi", "Asmat"]};
                        ...
                        <div id="view-form-service-${k}" class="card hidden">
                            ...
                            <div class="form-grid">
                                ...
                                <div class="form-group">
                                    ...
                                    <input id="inpProvince-${k}" list="listProvince-${k}" placeholder="Ketik atau pilih Provinsi" oninput="onProvinceChange('${k}')">
                                    <datalist id="listProvince-${k}">${Object.keys(PROVINCE_CITY_DATA).map(p => `<option value="${p}">`).join('')}</datalist>
                                </div>
                                <div class="form-group">
                                    <label>Kota</label>
                                    <input id="inpCity-${k}" list="listCity-${k}" placeholder="Pilih Provinsi dulu">
                                    <datalist id="listCity-${k}"></datalist>
                                </div>
                            ...
                            <div class="form-grid">
                                <div class="form-group">
                                    ...
                                    <input id="inpCategory-${k}" list="listCategory-${k}" placeholder="Ketik atau pilih Kategori" oninput="onCategoryChange('${k}')">
                                    <datalist id="listCategory-${k}">${PRODUCT_CATEGORIES.map(c => `<option value="${c}">`).join('')}</datalist>
                                </div>
                                <div class="form-group">
                                    <label>Model Alat</label>
                                    <input id="inpModel-${k}" list="listModel-${k}" placeholder="Pilih Kategori dulu">
                                    <datalist id="listModel-${k}"></datalist>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <input id="inpWarrantyStatus-${k}" list="listWarrantyStatus-${k}" placeholder="-- Belum Dipilih --" value="${k === 'pickup' ? '' : 'Out of Warranty'}">
                                    <datalist id="listWarrantyStatus-${k}">${opts(WARRANTY_STATUS_OPTIONS)}</datalist>
                                </div>
                                <div class="form-group">
                                    <label>Warranty Period (tahun)</label>
                                    <input id="inpWarrantyPeriod-${k}" list="listWarrantyPeriod-${k}" placeholder="Ketik atau pilih">
                                    <datalist id="listWarrantyPeriod-${k}">${opts(WARRANTY_PERIOD_OPTIONS)}</datalist>
                                </div>
                                <div class="form-group">
                                    <label>Asal Produk</label>
                                    <input id="inpOrigin-${k}" list="listOrigin-${k}" placeholder="Ketik atau pilih">
                                    <datalist id="listOrigin-${k}">${opts(PRODUCT_ORIGINS)}</datalist>
                                </div>
                            </div>
                            ...
                            <div class="form-grid">
                                ...
                                <div class="form-group">
                                    <label>Remarks</label>
                                    <input id="inpRemarks-${k}" list="listRemarks-${k}" placeholder="Ketik atau pilih">
                                    <datalist id="listRemarks-${k}">${opts(REMARKS_OPTIONS)}</datalist>
                                </div>
                                <div class="form-group">
                                    <label>Repair Status</label>
                                    <select id="inpStatus-${k}">${opts(repairStatusOptionsFor(k), 'Diterima')}</select>
                                </div>
                            ...
                            <div id="sparepartContainer-${k}"></div>
                            <div class="sparepart-empty-hint" id="sparepartEmptyHint-${k}" style="font-size:12px; color:#888; font-style:italic; padding:6px 0;">Belum ada sparepart ditambahkan.</div>
                            <div style="display:flex; gap:10px; margin-bottom:15px;">
                                <button type="button" class="btn btn-secondary" id="btnTambahPart-${k}" onclick="addSparepartSlot('${k}')">+ Tambah Part</button>
                                <button type="button" class="btn btn-secondary hidden" id="btnTambahItem-${k}" onclick="addSparepartSlot('${k}')">+ Tambah Item</button>
                            </div>
            ...
            function onProvinceChange(locKey) {
                ...
                document.getElementById(`listCity-${locKey}`).innerHTML = cities.map(c => `<option value="${c}">`).join('');
                document.getElementById(`inpCity-${locKey}`).placeholder = cities.length ? 'Ketik atau pilih Kota' : 'Pilih Provinsi dulu';
            }

            async function onCategoryChange(locKey) {
                const category = document.getElementById(`inpCategory-${locKey}`).value;
                const modelInput = document.getElementById(`inpModel-${locKey}`);
                const modelList = document.getElementById(`listModel-${locKey}`);

                if (!category) {
                    modelList.innerHTML = '';
                    modelInput.placeholder = 'Pilih Kategori dulu';
                    return;
                }

                if (category === 'Others') {
                    // Others: model alat tidak wajib ada di katalog - datalist dikosongkan,
                    // field tetap combobox yang sama tapi bebas diketik apa saja.
                    modelList.innerHTML = '';
                    modelInput.placeholder = 'Ketik nama model alat (kategori Others - bebas)';
                    return;
                }

                modelInput.placeholder = 'Memuat...';
                try {
                    const res = await authFetch(`/api/v1/device-models/?category=${encodeURIComponent(category)}`);
                    const models = res.ok ? await res.json() : [];
                    modelList.innerHTML = models.map(m => `<option value="${esc(m.model_name)}">`).join('');
                    modelInput.placeholder = models.length ? 'Ketik atau pilih Model' : '(Belum ada model utk kategori ini - tambah di Kelola Model Alat)';
                } catch(e) {
                    modelInput.placeholder = 'Gagal memuat model';
                }
            }

            let sparepartCounts = {}; // { [locKey]: jumlah sparepart yang sedang tampil (0-3) }
            const MAX_SPAREPART = 3;

            function addSparepartSlot(locKey) {
                const current = sparepartCounts[locKey] || 0;
                if (current >= MAX_SPAREPART) return;
                const n = current + 1;
                sparepartCounts[locKey] = n;

                document.getElementById(`sparepartEmptyHint-${locKey}`).classList.add('hidden');
                const container = document.getElementById(`sparepartContainer-${locKey}`);
                const div = document.createElement('div');
                div.className = 'sparepart-box';
                div.id = `sparepartBox${n}-${locKey}`;
                ...
                div.innerHTML = `
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
                        <span style="font-weight:bold; font-size:12px; color:#0056b3;">Sparepart ${n}</span>
                        <button type="button" class="btn btn-danger" style="padding:4px 9px; font-size:11px;" onclick="removeSparepartSlot('${locKey}', ${n})">Hapus</button>
                    </div>
                    <div class="form-grid">
                        <div class="form-group"><label>Nama Sparepart</label><input id="inpSpName${n}-${locKey}" placeholder="Nama sparepart"></div>
                        <div class="form-group"><label>Jumlah</label><input type="number" min="0" id="inpSpQty${n}-${locKey}"></div>
                        <div class="form-group"><label>Kode Sparepart</label><input id="inpSpCode${n}-${locKey}" placeholder="Kode/part number"></div>
                        <div class="form-group"><label>Harga (Rp)</label><input type="number" min="0" id="inpSpPrice${n}-${locKey}"></div>
                    </div>
                `;
                container.appendChild(div);
                updateSparepartButtons(locKey);
            }

            function removeSparepartSlot(locKey, n) {
                document.getElementById(`sparepartBox${n}-${locKey}`)?.remove();
                sparepartCounts[locKey] = Math.max((sparepartCounts[locKey] || 1) - 1, 0);
                if (sparepartCounts[locKey] === 0) {
                    document.getElementById(`sparepartEmptyHint-${locKey}`).classList.remove('hidden');
                }
                updateSparepartButtons(locKey);
            }

            function updateSparepartButtons(locKey) {
                const count = sparepartCounts[locKey] || 0;
                document.getElementById(`btnTambahPart-${locKey}`).classList.toggle('hidden', count > 0);
                document.getElementById(`btnTambahItem-${locKey}`).classList.toggle('hidden', count === 0 || count >= MAX_SPAREPART);
            }

            function clearSparepartSlots(locKey) {
                // Dipakai saat form dibuka utk tiket BARU (reset total) - hapus semua
                // kotak sparepart yang mungkin tersisa dari sesi sebelumnya.
                const container = document.getElementById(`sparepartContainer-${locKey}`);
                if (container) container.innerHTML = '';
                sparepartCounts[locKey] = 0;
                document.getElementById(`sparepartEmptyHint-${locKey}`).classList.remove('hidden');
                updateSparepartButtons(locKey);
            }
            ...
            async function fillTicketForm(locKey, t) {
                ...
                clearSparepartSlots(k);
                // Tentukan berapa slot yang perlu dibuka - sampai slot TERTINGGI yang
                // punya nama terisi (slot dibuka berurutan 1,2,3 - tidak bisa loncat).
                let neededSlots = 0;
                for (let n = 1; n <= 3; n++) {
                    if (spareparts.some(s => s.slot_no === n && s.name)) neededSlots = n;
                }
                for (let i = 0; i < neededSlots; i++) addSparepartSlot(k);
                [1, 2, 3].forEach(n => {
            ...
            function resetTicketForm(locKey) {
                ...
                setVal(`inpModel-${k}`, '');
                document.getElementById(`listModel-${k}`).innerHTML = '';
                document.getElementById(`inpModel-${k}`).placeholder = 'Pilih Kategori dulu';
